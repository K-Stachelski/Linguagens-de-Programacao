...
// Funcoes Obrigatoria: Insercao, Remocao e Atualizacao.  
    ...
    if(raiz == NULL){
        ...
        if(raiz->aluno->RGA == x){
            if(raiz->esq == NULL && raiz->dir == NULL){
                printf("\nInformacao do Aluno %s removida.\n", x, raiz->aluno->nome);
                return NULL;
            }
...
void imprimir_info(Aluno *aluno){
    printf("[Nome: %s - RGA: %d] ingressou em %d no curso[%d]", aluno->nome, aluno->RGA, aluno->ano_ingresso, aluno->codigo_curso);
}
